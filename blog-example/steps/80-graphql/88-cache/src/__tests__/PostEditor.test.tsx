...
test("renders correctly", () => {
  const editor = render(<PostEditor onSavePost={jest.fn()} />);
  expect(editor.asFragment()).toMatchSnapshot();
});
...
test("add post button callback", () => {
  ...
  expect(screen.getByText("Title correctly filled")).toBeInTheDocument();
  expect(screen.getByText("Body correctly filled")).toBeInTheDocument();
